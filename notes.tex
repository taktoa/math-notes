...
% Rank-select bitstrings / wavelet trees
% Discrete linear canonical transformation in O(n log n)
% Unification
% Simulated annealing
% Arnoldi iteration, vibrational analysis with generalized eigenvalue solvers
% Approximate nearest neighbor data structures
% Max clique in various graph classes
% Tarjan's SCC algorithm
% Derivative parsing for regular and context-free grammars
% Ukkonen's algorithm for constructing suffix trees
% Automatic differentiation
% Graham scan for finding convex hulls
% PageRank
% In-crowd algorithm for BPDN
% DPLL algorithm
...
% Spectral min-cut
% The simplex method
% Buchberger's algorithm for finding Grobner bases
% The heat method
% Crank-Nicolson method & solving the heat equation
% Thomas algorithm & solving Poisson's equation
% Goemans-Williamson max-cut via SDP, relationship between max-cut and Ising models

% @ Thomas algorithm (and applications to Poisson's equation)
% @@ {%
%   A \define{tridiagonal matrix} is a matrix that consists of all zeros except
%   for the diagonal and the two off-diagonals; they correspond to systems of
...
% }
% @@ {%
%   Tridiagonal matrix equations can be solved in $O(n)$ time, compared to the
%   usual $O(n^3)$ for Gaussian elimination.
% }
% @@ {%
%   A \define{block tridiagonal matrix} is a block matrix that is tridiagonal;
%   i.e.: a system of equations of the form:
%   \begin{equation*}
%   A_i \vec{x}_{i - 1} + B_i \vec{x}_i + C_i \vec{x}_{i + 1} = \vec{d}_i
%   \end{equation*}
% }
...
%@@ {%
%  The \define{Ising model} refers to a number of models of ferromagnetism in
%  which typically there is a magnet (which can either be pointed up or down) for
%  each element of $\mbb{Z}^2$. This can be conceptualized as a weighted graph
%  \begin{align*}
%    V & = I^2 \\
%    E & = \{(a, b) \in (I^2)^2 \mid \norm{a - b} = 1\} \\
%    w(i, j) & = 1 \\
%    G & = (V, E, w)
%  \end{align*}
%  where $I$ is some contiguous finite subset of $\mbb{Z}$.
%  %$f : \mbb{Z}^2 \to \{-1, 1\}$,
%}

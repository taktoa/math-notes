...
@@ {%
  Theorem: greedily coloring a graph using the reverse of a perfect elimination
  ordering always gives an optimal coloring of that graph.
}
@@ {%
...
@@ {%
  Theorem: there is an algorithm that, given a unit disk graph $G$, generates a
  coloring $\phi$ of $G$ such that $\mrm{order}(\phi) \le 3 \chi(G)$, i.e.: it is
  a 3-approximation algorithm for coloring unit disk graphs.
}

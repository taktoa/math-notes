...
\newcommand{\opcat}[1]{\ensuremath{{#1}^{\mathrm{op}}}}
...
@@ {%
  ...
  \textbf{Theorem}:
  if a graph $G$ has no edges ($G = ((V, \emptyset), \emptyset)$),
...
@ {%
  ...
  A graph $G = (V, E)$ is \define{regular} iff
  $\exists n \in \mbb{N} ~.~ \forall x \in V ~.~
  n = \indegree{}{G}{x} = \outdegree{}{G}{x}$. \\
  In other words, a graph is regular iff the outdegree of every node is equal
  to some natural $n$ and the indegree of every node is also equal to $n$.
}
@ {%
  The \define{indegree matrix} of an order-$n$ graph $G$ is given by
  $\msf{D}_G^\msf{in} = \diag(\indegree{}{G}{1}, \dotsc, \indegree{}{G}{n})$.

  The \define{outdegree matrix} of an order-$n$ graph $G$ is given by
  $\msf{D}_G^\msf{out} = \diag(\outdegree{}{G}{1}, \dotsc, \outdegree{}{G}{n})$.
...
@ Laplacian matrix
@@ {%
  The \define{Laplacian matrix} of a graph $(V \in \catset, E \subseteq V^2)$
...
@@ {%
  \textbf{Theorem}: if $G = ([1, n], E)$ is an unweighted undirected graph, then
  the unit eigenvectors of $\msf{Adj}(G)$ form an orthogonal basis for $R^n$.
}
@@ {%
  \textbf{Theorem}:
  If $G = ((V, E), \delta)$, then
  $\msf{Spec}(\overline{G}) = \{\card(V) \}$
}
\end{easylist}
